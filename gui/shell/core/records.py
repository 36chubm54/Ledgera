...
from gui.shell.core.support import build_record_tree_values
from gui.tooltip import show_popup_tooltip
from utils.records.tags import color_for_tag
    ...
    *,
    ...
    for kind, color in kind_to_foreground.items():
        try:
            ...
            logger.debug("Records tree tag style skipped for %s: %s", kind, error)
    ...
    for item in list_items:
        ...
        if tag_color:
            ...
            try:
                ...
                logger.debug("Record tags tree style skipped for %s: %s", tag_style, error)
...
def destroy_records_tooltip_window(tooltip_window: tk.Toplevel | None) -> None:
    ...
    try:
        ...
        logger.debug("Records tooltip cleanup skipped: %s", error)
