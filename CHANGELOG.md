...
- Added the beta.1 Kotlin Budget screen through Rust/Kotlin UniFFI with category/tag budgets, mandatory-expense inclusion, overlap-safe periods, spend tracking, pace statuses, and forecasts.
- Added beta.1 full JSON backup/restore through Rust/Kotlin UniFFI; budgets are included in atomic full-database snapshots with legacy category-scope compatibility and preview-before-restore confirmation in Settings.
- Added SHA-256 checksum generation and verification for new full backups, strict Budget money-field validation, canonical tag-budget scopes, and cross-tab refresh after restore.
