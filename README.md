...
Текущая alpha-версия `v2.0.0-alpha.2` завершает breaking-change переход на двухуровневую валютную модель и доводит архитектурную чистку верхних слоёв: значения в БД теперь хранятся как `amount_base` / `limit_base` в `base_currency`, `display_currency` управляет только отображением в UI, а shell/runtime orchestration вынесен из основных entry-point модулей в более узкие helpers.
...
- Базовая валюта в текущем alpha-конфиге по умолчанию — `KZT`
